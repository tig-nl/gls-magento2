...
REPOSITORY_CONFIG="{\"type\": \"package\",\"package\": { \"name\": \"tig/gls\", \"version\": \"master\", \"dist\": {\"type\": \"zip\",\"url\": \"${TRAVIS_BUILD_DIR}/build.zip\",\"reference\": \"master\" }, \"autoload\": {\"files\": [\"registration.php\"],\"psr-4\": {\"TIG\\\\GLS\\\\\": \"\"}}}}"
...
( cd "${BUILD_DIR}/" && composer config repositories.gls "${REPOSITORY_CONFIG}" )
...
( cd "${BUILD_DIR}/" && composer require tig/gls --ignore-platform-reqs )
...
mysql -u${MAGENTO_DB_USER} ${MYSQLPASS} -h${MAGENTO_DB_HOST} -P${MAGENTO_DB_PORT} ${MAGENTO_DB_NAME} < Test/Fixtures/tig-gls-fixture.sql
